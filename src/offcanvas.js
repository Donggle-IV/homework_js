document.addEventListener('DOMContentLoaded', () => {
  ...
  function setActiveLink() {
    ...
    const navLinks = document.querySelectorAll('.header__nav-link, .header__off-canvas-nav-link');
    
    navLinks.forEach(link => {
      if (link.getAttribute('href') === currentPath) {
        link.classList.add('header__nav-link--active', 'header__off-canvas-nav-link--active');
      } else {
        link.classList.remove('header__nav-link--active', 'header__off-canvas-nav-link--active');
      }
    });
  }
  ...
  document.addEventListener('keydown', (e) => {
    if (e.key === 'Escape' && offCanvas.classList.contains('header__off-canvas-wrapper--active')) {
      closeMenuHandler();
    }
  });
});